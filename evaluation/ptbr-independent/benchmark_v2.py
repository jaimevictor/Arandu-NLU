...
from freeze_v2 import MANIFEST, sha256_file, verify_manifest
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- RSS máximo: {summary['rss']['max_bytes']} bytes ({summary['rss']['collection']})",
        f"- HWM máximo: {summary['hwm']['max_bytes']} bytes ({summary['hwm']['collection']})",
        "",
...
def main() -> None:
    ...
    semantic_catalog, cases = load_fixtures(ROOT, version=2)
    product_catalog = load_json((ROOT / "data" / "catalog-v2.json").read_text(encoding="utf-8"))
    server = start_local(binary)
    rss_samples: list[int] = []
    hwm_samples: list[int] = []
    try:
        ...
        for _ in range(MEASURED_ROUNDS):
            ...
            if memory["rss_bytes"] is not None:
                rss_samples.append(memory["rss_bytes"])
            if memory["hwm_bytes"] is not None:
                hwm_samples.append(memory["hwm_bytes"])
    finally:
        server.close()
    ...
    cold_rss = [sample["rss_bytes"] for sample in cold if sample.get("rss_bytes") is not None]
    cold_hwm = [sample["hwm_bytes"] for sample in cold if sample.get("hwm_bytes") is not None]
    report = {
        "schema_version": 1,
        ...
        "hashes": {
            ...
            "benchmark": sha256_file(ROOT / "benchmark_v2.py"),
        ...
        "summary": {
            ...
            "rss": {
                "process": "local-nlu serve",
                "unit": "bytes",
                "samples": rss_samples + cold_rss,
                "max_bytes": max(rss_samples + cold_rss) if rss_samples + cold_rss else None,
                "collection": "Linux /proc/<pid>/status VmRSS",
            },
            "hwm": {
                "process": "local-nlu serve",
                "unit": "bytes",
                "samples": hwm_samples + cold_hwm,
                "max_bytes": max(hwm_samples + cold_hwm) if hwm_samples + cold_hwm else None,
                "collection": "Linux /proc/<pid>/status VmHWM",
            },
        },
    }
    output_dir = arguments.output_dir.resolve()
    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / "benchmark-v2.json").write_text(
        json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    (output_dir / "benchmark-v2.md").write_text(render_markdown(report), encoding="utf-8")
